...
export function SocialLoginButtons() {
  const handleSocial = async (provider: OAuthProvider) => {
    try {
      ...
      const message = e instanceof Error ? e.message : `Could not sign in with ${provider}`;
      toast.error(message);
    }
  };
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      {providers.map((p) => (
        <Button key={p.id} variant="outline" onClick={() => handleSocial(p.id)} type="button" className="gap-2">
          {p.icon}
          {p.label}
        </Button>
      ))}
